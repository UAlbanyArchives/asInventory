								...
								if not row[22].value is None:
									...
									if postAO.status_code == 200:
										...
										if not str(row[22].value).strip().lower().startswith("http"):
											...
											# get dao paths from local_settings.cfg
											try:
												webDir = config.get('asInventory', 'daoDirectory')
												webLink = config.get('asInventory', 'daoURL')
												if not os.path.isdir(webDir):
													raise ValueError("Error reading daoDirectory from local_settings.cfg. " + webDir + " is not a valid directory.")
											except:
												raise ValueError("Error reading dao paths from local_settings.cfg")
